      ...
      <div className='container mx-auto py-8 mt-24'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
          {anonymousPosts.map((post) => (
            <div
              key={post._id}
              className='bg-gradient-to-br from-blue-gray-light to-blue-gray border border-gray-200 shadow-sm rounded-xl overflow-hidden'
            >
              <div className='rounded-t-xl'>
                <p className='text-sm text-gray-600'>{post.featured && 'Featured'}</p>
              </div>
              <div className='p-4 md:p-5'>
                <h3 className='text-lg font-bold text-gray-800'>{post.title}</h3>
                <p className='mt-2 text-gray-600'>{post.article}</p>
